...
function StaffCard({
  ...
  const fetchAppts = useCallback(async () => {
    ...
    const { data } = await supabase
      .from("appointments")
      .select("id,name,vehicle,start_time,end_time,appointment_date,status")
      .eq("staff_id", s.id)
    ...
    <div className={`bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden flex flex-col transition-opacity ${!s.active ? "opacity-50" : ""}`}>
      ...
      <div className="p-5 flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {s.specialty && (
            <p className="text-xs text-gray-500 mt-0.5">{s.specialty}</p>
          )}
          {s.phone && (
      ...
      {expanded && (
        <div className="border-t border-gray-100">
          {loadingAppts && (
            <p className="text-xs text-gray-400 text-center py-4 animate-pulse">Cargando...</p>
          )}
